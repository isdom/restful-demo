...
public class WebSocketIndexPageHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public WebSocketIndexPageHandler(final String websocketPath) {
    ...
    protected void channelRead0(final ChannelHandlerContext ctx, final FullHttpRequest req) throws Exception {
        ...
        if ("/".equals(req.uri()) || "/index.html".equals(req.uri())) {
            final String webSocketLocation = getWebSocketLocation(ctx.pipeline(), req, websocketPath);
            final ByteBuf content = WebSocketServerIndexPage.getContent(webSocketLocation);
